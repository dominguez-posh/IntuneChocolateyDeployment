...
###Install And Detection Script Creation
        ...
        try{Remove-Item ($WorkPath + "\install.intunewin") -ErrorAction Stop}catch{}
...
try{Remove-Item ($WorkPath + "\install.intunewin") -ErrorAction Stop}catch{"Some Error 231"}
...
if($IconName -like "*pending system reboot*"){write-host "reboot is pending, please update and reboot and then try again!";Read-Host}
