...
function Install-IntunechocoComponents {
        ...
        ###Install And Detection Script Creation
                ...
                #write-host $WorkPath
                #Remove-Item ("\\?\" + $env:temp + "\intunedeployment\install.intunewin")
                try{Remove-Item ($UNCWorkPath + "\intunedeployment\install.intunewin") }catch{"Some Error 104"}
                #exit
        ...
        ### Deploy Application
                ...
                write-host $IntuneWinFile.Path
                write-host $AppName
                write-host $Infos.Description
                write-host $Infos.SoftwareSite
                write-host $Infos.SoftwareLicense
        
                if($Infos.SoftwareLicense = "n/a"){
                  $Infos.SoftwareLicense = $Infos.SoftwareSite 
                  }
                #$Infos.Description = "-"
                #$Infos.SoftwareLicense = "-"
                #write-host "-FilePath $IntuneWinFile.Path -DisplayName $AppName -Description $Infos.Description -Notes "PoweredByChocolatey" -Publisher "-" -InformationURL $Infos.SoftwareSite -PrivacyURL $Infos.SoftwareLicense -InstallExperience system -RestartBehavior suppress -DetectionRule $DetectionScript -RequirementRule $RequirementRule -InstallCommandLine $InstallCommandLine -UninstallCommandLine $UninstallCommandLine -Icon $Icon"
                #write-host $Infos.Description
                #exit
                #try{$IntuneApp = Add-IntuneWin32App -FilePath $IntuneWinFile.Path -DisplayName $AppName -Description $Infos.Description -Notes "PoweredByChocolatey" -Publisher "-" -InformationURL "-" -PrivacyURL "-" -InstallExperience system -RestartBehavior suppress -DetectionRule $DetectionScript -RequirementRule $RequirementRule -InstallCommandLine $InstallCommandLine -UninstallCommandLine $UninstallCommandLine }catch{"Some Error 131"}
                #$IntuneApp = Add-IntuneWin32App -FilePath $IntuneWinFile.Path -DisplayName $AppName -Description $Infos.Description -Notes "PoweredByChocolatey" -Publisher "-" -InformationURL $Infos.SoftwareSite -PrivacyURL "https://www.xnview.com/PrivacyPolicyApps.html" -InstallExperience system -RestartBehavior suppress -DetectionRule $DetectionScript -RequirementRule $RequirementRule -InstallCommandLine $InstallCommandLine -UninstallCommandLine $UninstallCommandLine -Icon $Icon
                
                #without try
                $IntuneApp = Add-IntuneWin32App -FilePath $IntuneWinFile.Path -DisplayName $AppName -Description $Infos.Description -Notes "PoweredByChocolatey" -Publisher "-" -InformationURL $Infos.SoftwareSite -PrivacyURL $Infos.SoftwareLicense -InstallExperience system -RestartBehavior suppress -DetectionRule $DetectionScript -RequirementRule $RequirementRule -InstallCommandLine $InstallCommandLine -UninstallCommandLine $UninstallCommandLine -Icon $Icon 
        
                #with try
                #try{$IntuneApp = Add-IntuneWin32App -FilePath $IntuneWinFile.Path -DisplayName $AppName -Description $Infos.Description -Notes "PoweredByChocolatey" -Publisher "-" -InformationURL $Infos.SoftwareSite -PrivacyURL $Infos.SoftwareLicense -InstallExperience system -RestartBehavior suppress -DetectionRule $DetectionScript -RequirementRule $RequirementRule -InstallCommandLine $InstallCommandLine -UninstallCommandLine $UninstallCommandLine -Icon $Icon }catch{Write-Host "Some error 131:"
                #Write-Host $_}
        ...
        try{Remove-Item ($UNCWorkPath + "\intunedeployment\install.intunewin")}catch{"Some Error 231"}
        ...
        if($IconName -like "*pending system reboot*"){write-host "reboot is pending, please update and reboot and then try again!";Read-Host;exit}
        ...
        $Content = '
        "Adobe Acrobat Reader DC (choco)","adobereader"
        "Google Chrome (choco)","googlechrome"
        "Mozilla Firefox (choco)","firefox"
        "Java SE Runtime Environment (choco)","jre8"
        "Microsoft Visual C++ Redistributable for Visual Studio 2015-2022 (choco)","vcredist140"
        "Notepad++ (choco)","notepadplusplus"
        "Python 3.x (choco)","python3"
        "Python 2.x (choco)","python2"
        "Adobe AIR Runtime (choco)","adobeair"
        "7-Zip (choco)","7zip.install"
        "Microsoft Visual C++ Redistributable for Visual Studio 2015 (choco)","vcredist2015"
        "Microsoft .NET Framework 4.8 (choco)","dotnetfx"
        "VLC media player (choco)","vlc"
        "Git (choco)","git.install"
        "Microsoft Edge (choco)","microsoft-edge"
        "Zoom Client for Meetings (choco)","zoom"
        "Microsoft Silverlight (choco)","silverlight"
        "Sysinternals (choco)","sysinternals"
        "Foxit PDF Reader (choco)","foxitreader"
        "Paint.NET (choco)","paint.net"
        "Teamviewer.install (choco)","teamviewer"
        "Skype (choco)","skype"
        "Putty (choco)","putty.install"
        "Java SE (choco)","jdk8"
        "Visual Studio Code (choco)","vscode"
        "FileZilla (choco)","filezilla"
        "PDF Creator (choco)","pdfcreator"
        "Dot Net 3.5 (choco)","dotnet3.5"
        "Dot Net 4.5.2 (choco)","dotnet4.5.2"
        "Windows Terminal (choco)","microsoft-windows-terminal"
        "WinSCP (choco)","winscp.install"
        "Inkscape (choco)","inkscape"
        "Dropbox (choco)","dropbox"
        "Wireshark (choco)","wireshark"
        "Process Explorer (choco)","procexp"
        "Advanced IP Scanner (choco)","advanced-ip-scanner"
        "Opera (choco)","opera"
        "Everything (choco)","everything"
        "Microsoft OneDrive Sync Client (choco)","onedrive"
        "Teamviewer Host (choco)","teamviewer.host"
        "Teamviewer QuickSupport (choco)","teamviewer-qs"
        "WinDirStat (choco)","windirstat"
        "Adobe Creative Cloud Client (choco)","adobe-creative-cloud"
        "Greenshot (choco)","greenshot"
        "Process Monitor (choco)","procmon"
        "PSExec (choco)","psexec"
        "Sublime Text 4 (choco)","sublimetext4"
        "Power BI Desktop (choco)","powerbi"
        "Rufus (choco)","rufus"
        "Veeam Agent for Microsoft Windows (choco)","veeam-agent"
        ...
        "Brackets","brackets"
        "CDex","cdex"
        "Dell Support Assist","supportassist"
        "Geogebra classic","geogebra-classic.install"
        "GoogleEarth","googleearth"
        "GPG4Win","gpg4win"
        "HP SupportAssistant","hpsupportassistant"
        "intel-dsa","intel-dsa"
        "keepassxc","keepassxc"
        "krita","krita"
        "lenovo-thinkvantage-system-update","lenovo-thinkvantage-system-update"
        "musescore","musescore"
        "shotcut.install","shotcut.install"
        "spotify","spotify"
        "tipp10","tipp10"
        "vscodium.install","vscodium.install"
        ...
        "git","git.install"
        "zoom","zoom"
        "dellcommandupdate","dellcommandupdate"
        ...
        "notenmanager-heimversion","notenmanager-heimversion"
        '
